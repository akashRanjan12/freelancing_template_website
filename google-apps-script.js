...
function doPost(e) {
  try {
    ...
    try {
      ...
      // Try MailApp first, fallback to GmailApp
      try {
        MailApp.sendEmail({
          to: recipient,
          subject: subject,
...
// =========================================================================
// RUN THIS FUNCTION ONCE IN APPS SCRIPT EDITOR TO AUTHORIZE EMAIL PERMISSION!
// In top toolbar: select 'authorizeAndTestEmail' -> Click 'Run' -> Click 'Review Permissions' -> 'Allow'
// =========================================================================
function authorizeAndTestEmail() {
  var recipient = "gravityanti87@gmail.com";
  MailApp.sendEmail(
    recipient,
    "Website Contact Form Email Test",
    "Email delivery is working! Any new contact submissions on your website will now be delivered to this inbox automatically."
  );
